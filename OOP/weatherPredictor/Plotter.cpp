...
void plotCandlesticks(const std::vector<Candlestick>& candlesticks, TimeFrame timeframe, int chartHeight) {
    ...
    std::vector<Candlestick> displayData = Internal::applyIntelligentSampling(candlesticks, timeframe);
    bool wasDownsampled = displayData.size() < candlesticks.size();

    PlotConfiguration config = Internal::determineOptimalCompression(displayData.size(), timeframe);

    double minTemp = Internal::findMinTemperature(displayData);
    double maxTemp = Internal::findMaxTemperature(displayData);
    ...
    Internal::printChartHeader(config, wasDownsampled, candlesticks.size(), displayData.size());
    ...
    for (int row = 0; row < chartHeight; ++row) {
        ...
        for (const auto& candle : displayData) {
            std::cout << Internal::getCandlestickAtRow(candle, currentTemp, tempPerRow, config) 
                      << config.spacingStr;
        }
        std::cout << "\n";
    }
    
    Internal::printXAxisLabels(displayData, timeframe, config);
    Internal::printSummary(displayData);
}
...
std::string getCandlestickAtRow(const Candlestick& candle, double rowTemp, double tempPerRow, const PlotConfiguration& config) {
    ...
    if (rowTemp <= bodyTop + tolerance && rowTemp >= bodyBottom - tolerance) {
        bool isUpTrend = candle.isUptrend();
